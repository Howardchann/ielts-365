...
function buildLabel() {
  ...
  if (BUILD && BUILD.sha) parts.push(BUILD.sha + (BUILD.dirty ? '*' : ''));
...
Page({
  ...
  refreshCloud(){
    ...
    else{const d=new Date(sr.lastSyncAt),today=new Date();const same=d.toDateString()===today.toDateString();meta='上次同步：'+(same?'今天 ':(p(d.getMonth()+1)+'-'+p(d.getDate())+' '))+p(d.getHours())+':'+p(d.getMinutes());}
    if(sr.enabled&&sr.pending>0) meta+='，待同步 '+sr.pending+' 条复习记录';
    theme.sameSet(this,{cloudOn:sr.enabled,syncing:!!sr.syncing,cloudMeta:meta,cloudErr:sr.error||'',cloudTip:sr.enabled?'进度自动同步，多台设备共用一份进度':'开启后进度自动同步，多台设备共用一份进度'});
  },
