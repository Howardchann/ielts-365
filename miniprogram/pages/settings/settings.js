...
Page({
  ...
  onExport(){let text;try{text=store.exportBackup();}catch(e){wx.showToast({title:'导出失败',icon:'none'});return;}const s=this.backupSummaryOf(text);this.setData({backupText:text,showBackup:true,showImport:false,importText:''});wx.setClipboardData({data:text,success:()=>wx.showModal({title:'备份已导出',content:(s?s+'\n\n':'')+'全文已复制到剪贴板，粘贴到备忘录等处保存即可。',showCancel:false,confirmColor:'#176B5B'}),fail:()=>wx.showToast({title:'复制失败，请长按选中文本手动复制',icon:'none',duration:2500})});},
  ...
  doImport(mode){const text=(this.data.importText||'').trim();if(!text){wx.showToast({title:'请先粘贴备份内容',icon:'none'});return;}let r;try{r=store.importBackup(text,mode);}catch(e){wx.showModal({title:'导入失败',content:e.message||String(e),showCancel:false,confirmColor:'#176B5B'});return;}this.setData({showImport:false,importText:''});this.refresh();const bs=this.backupSummaryOf(text);const head=mode==='merge'?'合并完成':'覆盖完成';const detail=(bs?('该备份：'+bs+'\n\n'):'')+(mode==='merge'?('新增 '+r.addedDays+' 天打卡、'+r.addedWords+' 个重点词\n合并后共 '+r.totalDays+' 天、'+r.totalWords+' 个重点词\n已恢复 '+r.reviewWords+' 个复习记录'):('已替换为备份中的 '+r.totalDays+' 天打卡、'+r.totalWords+' 个重点词、'+r.reviewWords+' 个复习记录'));wx.showModal({title:head,content:detail,showCancel:false,confirmColor:'#176B5B'});},
  ...
  onSyncNow(){if(this.data.syncing)return;this.setData({syncing:true});store.syncNow().then(r=>{this.refreshCloud();wx.showModal({title:r.ok?'同步完成':'同步失败',content:r.ok?('进度已与云端对齐。\n'+(r.msg||'')):((r.msg||'未知错误')+'\n\n本地进度不受影响，可稍后点「立即同步」重试。'),showCancel:false,confirmColor:'#176B5B'});});},
  onAbout(){wx.showModal({title:ABOUT_MODAL_TITLE,content:ABOUT_TEXT,showCancel:false,confirmColor:'#176B5B'});}
});
