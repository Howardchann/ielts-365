...
**待清理（附带发现，未动手）**：`dist_g/g` 下有 **313 个 djb2 负数哈希残留文件**（形如 `-1c1d0f5.mp3`，云函数白名单 `^g/[0-9]/[0-9a-f]{8}s?\.mp3$` 不认），是第一批命名 bug 的遗留物。`clean_g_extra.py` / `g_cleanup_plan.json` 之前只出过清单，未执行删除。
