...
| 12 | 关掉同步开关后不再上送但本机记录不受影响；**重新打开后一次补齐**（`setSyncEnabled(true)` 内部直接 `syncNow()`，攒下的 dirty 全部上云、`pending` 归零） |
...
2026-09-24 实测结论（两次变异，各持一条被测路径）：

| 变异 | 改法 | 结果 |
|---|---|---|
| 去掉 in-flight 修复 | `applyMerged` 里改回无条件 `delete data.reviewDirty[w]` | 【13】**精准失败 4 项**；【14】仍过（它测服务端，未被这次变异影响），符合预期 |
| 去掉重开开关的补齐 | `setSyncEnabled` 里把 `return syncNow()` 换成 `Promise.resolve(...)` | 【12】新增的 3 项**精准失败**（云端记录数不变、`pending` 停在 1） |

两次都证明对应用例真能抓到 bug，不是空测。**加完用例就该做一遍这种反向验证** ——
否则你无法分辨"用例通过"和"用例根本没走到被测代码"。
